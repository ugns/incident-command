...
resource "aws_api_gateway_resource" "activitylog_id" {
  rest_api_id = aws_api_gateway_rest_api.incident_cmd.id
...
resource "aws_api_gateway_method" "activitylog_id_get" {
  rest_api_id   = aws_api_gateway_rest_api.incident_cmd.id
  resource_id   = aws_api_gateway_resource.activitylog_id.id
  http_method   = "GET"
  authorization = "NONE"
}
resource "aws_api_gateway_integration" "activitylog_id_get" {
  depends_on              = [aws_api_gateway_method.activitylog_id_get]
  rest_api_id             = aws_api_gateway_rest_api.incident_cmd.id
  resource_id             = aws_api_gateway_resource.activitylog_id.id
  http_method             = aws_api_gateway_method.activitylog_id_get.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.activitylogs.invoke_arn
}
resource "aws_lambda_permission" "apigw_activitylog_id_get" {
...
resource "aws_api_gateway_method" "activitylog_id_options" {
  rest_api_id   = aws_api_gateway_rest_api.incident_cmd.id
  resource_id   = aws_api_gateway_resource.activitylog_id.id
  http_method   = "OPTIONS"
  authorization = "NONE"
}
resource "aws_api_gateway_integration" "activitylog_id_options" {
  depends_on  = [aws_api_gateway_method.activitylog_id_options]
  rest_api_id = aws_api_gateway_rest_api.incident_cmd.id
  resource_id = aws_api_gateway_resource.activitylog_id.id
  http_method = aws_api_gateway_method.activitylog_id_options.http_method
  type        = "MOCK"
  request_templates = {
    "application/json" = "{\"statusCode\": 200}"
  }
}
resource "aws_api_gateway_method_response" "activitylog_id_options" {
  rest_api_id = aws_api_gateway_rest_api.incident_cmd.id
  resource_id = aws_api_gateway_resource.activitylog_id.id
  http_method = aws_api_gateway_method.activitylog_id_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "activitylog_id_options" {
  depends_on  = [aws_api_gateway_integration.activitylog_id_options]
  rest_api_id = aws_api_gateway_rest_api.incident_cmd.id
  resource_id = aws_api_gateway_resource.activitylog_id.id
  http_method = aws_api_gateway_method.activitylog_id_options.http_method
  status_code = aws_api_gateway_method_response.activitylog_id_options.status_code
  response_templates = {
    "application/json" = ""
  }
  response_parameters = {
